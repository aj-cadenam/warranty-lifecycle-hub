...
class ProcesarCorreo:
    ...
    def _procesar(self, correo: CorreoEntrante) -> dict:
        ...
        if decision.tipo == TipoCorreo.ACTA_ENTREGA:
            resultado["accion_tomada"] = self._manejar_acta(correo, decision)

        elif decision.tipo in (TipoCorreo.ACTUALIZACION_PROVEEDOR, TipoCorreo.CONFIRMACION_DESPACHO):
            resultado["accion_tomada"] = self._manejar_actualizacion(correo, decision)
    ...
    def _manejar_acta(self, correo: CorreoEntrante, decision: DecisionCorreo) -> str:
        # Si el correo trae PDF adjunto, procesarlo como documento
        for pdf_path in correo.adjuntos:
            ...
            procesador.execute(pdf_path=pdf_path)
            return "solicitud_creada_desde_pdf"

        # Sin adjunto: crear solicitud directamente con info del correo
        if decision.equipo_serial:
            try:
                ...
                return "solicitud_creada_desde_correo"
            except ValueError:
                return "equipo_sin_garantia_vigente"
        return "sin_serial_identificado"

    def _manejar_actualizacion(self, correo: CorreoEntrante, decision: DecisionCorreo) -> str:
        if not decision.equipo_serial:
            return "sin_serial_identificado"

        solicitudes = self._solicitud_repo.find_by_estado(EstadoSolicitud.DESPACHADA)
        solicitudes += self._solicitud_repo.find_by_estado(EstadoSolicitud.EN_REPARACION)
        solicitud = next((s for s in solicitudes if s.equipo_id == decision.equipo_serial), None)

        if not solicitud:
            return "solicitud_activa_no_encontrada"
